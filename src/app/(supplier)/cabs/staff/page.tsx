...
export default function CabStaffPage() {
    ...
    const addStaffMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['cab-staff'] });
    ...
    return (
        <div>
            <Topbar title="Operations Core" subtitle="Elite staff management for your premium cab network" />
            <div className="p-6 space-y-8 animate-fadeIn">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex gap-6">
                        {[
                            { label: 'Network Staff', value: staff.length, icon: Fingerprint, color: 'hsl(var(--accent))' },
                            { label: 'Authorized Units', value: staff.filter((s: any) => s.isActive).length, icon: Shield, color: 'hsl(var(--success))' },
                        ].map((stat, i) => (
                        ...
                        <div className="w-[400px] glass-card p-10 h-fit sticky top-6 animate-fadeIn overflow-hidden border-accent/20">
                            ...
                            <div className="space-y-6 mb-10 relative z-10">
                                ...
                                <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/03 border border-white/05">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-0.5">Direct Line</div>
                            ...
                            <div className="relative z-10">
                                <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground mb-6 flex items-center gap-3">
                                    <Lock size={14} className="text-accent" /> Security Protocol
                                </h4>
                            ...
                            <div className="grid grid-cols-1 gap-3 relative z-10">
                                ...
                                {selectedStaff.role !== 'OWNER' && (
                                    <div className="flex gap-3">
                                        ...
                                        >
                                            <UserX size={16} className="inline mr-2" /> {selectedStaff.isActive ? 'Suspend' : 'Reinstate'}
                                        </button>
                                        <button
                        ...
                        >
                            ...
                            >
                                ...
                                <div className="flex items-center gap-4 relative z-10">
                                    ...
                                    <div>
                                        <h2 className="text-2xl font-black tracking-tight text-white leading-none">Provision Staff</h2>
                                        <p className="text-white/60 text-[10px] font-black uppercase tracking-[0.2em] mt-2 italic">Infrastructure Access Control</p>
                                    </div>
                                </div>
                                <button 
                            ...
                            <form className="p-10 space-y-10" onSubmit={(e) => {
                                ...
                                addStaffMutation.mutate({
                                    ...
                                    permissions: {
                                        ...
                                        drivers_manage: formData.get('role') === 'MANAGER',
                                    }
                                });
                            }}>
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 ml-1">Full Legal Name</label>
                                    <input name="name" required className="w-full bg-white/05 border border-white/05 rounded-2xl px-6 py-4.5 text-sm font-bold text-white focus:ring-4 focus:ring-blue-500/20 outline-none transition-all placeholder:text-white/10 shadow-inner" placeholder="e.g. Ishaan Deshmukh" />
                                </div>

                                <div className="space-y-3">
                                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 ml-1">Corporate Access Email</label>
                                    <input name="email" type="email" required className="w-full bg-white/05 border border-white/05 rounded-2xl px-6 py-4.5 text-sm font-bold text-white focus:ring-4 focus:ring-blue-500/20 outline-none transition-all placeholder:text-white/10 shadow-inner" placeholder="ishaan@starkings.cab" />
                                </div>

                                <div className="grid grid-cols-2 gap-10">
                                    <div className="space-y-3">
                                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 ml-1">Direct Contact No.</label>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="relative">
                                            <select name="role" required className="w-full bg-white/05 border border-white/05 rounded-2xl px-6 py-4.5 text-sm font-bold text-white focus:ring-4 focus:ring-blue-500/20 outline-none transition-all appearance-none cursor-pointer shadow-inner">
                                                <option className="bg-[#0d101b]" value="RECEPTIONIST">Unit Receptionist</option>
                                ...
                                <div className="flex gap-6 pt-6">
                                    ...
                                    >
                                        ...
                                        <span className="relative z-10">{addStaffMutation.isPending ? 'Provisioning...' : 'Confirm Access'}</span>
                                    </button>
                                </div>
                            </form>
                        </div>
